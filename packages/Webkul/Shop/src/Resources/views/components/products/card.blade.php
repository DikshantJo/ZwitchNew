...
@pushOnce('scripts')
    ...
    >
        <!-- Grid Card -->
        <div
            class="1180:transtion-all group w-full rounded-md 1180:relative 1180:grid 1180:content-start 1180:overflow-hidden 1180:duration-300 1180:hover:shadow-[0_5px_10px_rgba(0,0,0,0.1)] bg-white dark:bg-[#1a1a1a]"
            v-if="mode != 'list'"
        >
            ...
            <div class="-mt-9 grid max-w-[291px] content-start gap-2.5 bg-white dark:bg-[#1a1a1a] p-2.5 transition-transform duration-300 ease-out group-hover:-translate-y-0 group-hover:rounded-t-lg max-md:relative max-md:mt-0 max-md:translate-y-0 max-md:gap-0 max-md:px-0 max-md:py-1.5 max-sm:min-w-[170px] max-sm:max-w-[192px] z-10">
                ...
                <div
                    class="flex items-center justify-between text-lg font-semibold max-sm:text-sm max-sm:leading-6"
                >
                    <div v-html="product.price_html"></div>
                    
                    <!-- Heart Icon Next to Price -->
                    {!! view_render_event('bagisto.shop.components.products.card.wishlist_option.before') !!}
                ...
                <div class="action-items flex items-center justify-between opacity-0 transition-all duration-300 ease-in-out group-hover:opacity-100 max-md:hidden">
                    @if (core()->getConfigData('sales.checkout.shopping_cart.cart_page'))
                        ...
                        <button
                            class="secondary-button w-full max-w-full p-2.5 text-sm font-medium max-sm:rounded-xl max-sm:p-2 hidden"
