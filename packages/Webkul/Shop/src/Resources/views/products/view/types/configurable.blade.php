            ...
            app.component('v-product-configurable-options', {
                ...
                methods: {
                    ...
                    autoSelectFirstOptions() {
                        this.childAttributes.forEach((attribute, index) => {
                            // Check if there are valid options (more than just the placeholder)
                            if (attribute.options && attribute.options.length > 1) {
                                // Skip the first option which is usually "Select Options"
                                const firstValidOption = attribute.options[1];
                                if (firstValidOption && firstValidOption.id) {
                                    this.configure(attribute, firstValidOption.id);
                                }
                            }
                        });
                    },
                    ...
                    fillAttributeOptions(attribute) {
                        ...
                        for (let i = 0; i < options.length; i++) {
                            ...
                            // For single options, always add them even if allowedProducts is empty
                            if (allowedProducts.length > 0 || options.length === 1) {
